...
const calcPricePercentage = (filter: Filter, price: number) => {
  ...
  const diffScale = d3.scaleLinear().domain([0, diffPrice]).range([100, 0]);
...
const calcModePercentage = (
  filter: Filter,
  modes: { isSinglePlayer: boolean; isMultiPlayer: boolean }
) => {
  let modeCount = 0;
  Object.keys(modes).forEach((mode: string) => {
    if (
      (mode === "isSinglePlayer" && filter.Mode.isSinglePlayer) ||
      (mode === "isMultiPlayer" && filter.Mode.isMultiPlayer)
    ) {
      modeCount++;
    }
  });
...
export const calcDevicePercentage = (
  filter: Filter,
  devices: SteamDeviceType
) => {
  let deviceCount = 0;
  Object.keys(devices).forEach((device: string) => {
    if (
      (device === "windows" && filter.Device.windows) ||
      (device === "mac" && filter.Device.mac)
    ) {
      deviceCount++;
    }
  });
...
export const calcAllMatchPercentage = (
  filter: Filter,
  data: {
    genres: string[];
    price: number;
  ...
  const modeMatchPercent = calcModePercentage(filter, {
