            ...
            {friendsOwnGames.length > 0 ? (
              descFriendsData.map((game: GetFriendGamesResponse, index: number) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <div className={`p-2`} onClick={() => handleGameInNode(game)}>
                        {game.gameName}
                        {isGameInNode(game) ?
                          <IconButton size='small' sx={{ color:'white' }} onClick={() => setSelectedIndex(nodes.findIndex((node) => node.title == game.gameName))}>
                              <SearchIcon />
                          </IconButton>
                        : null}
                      </div>
