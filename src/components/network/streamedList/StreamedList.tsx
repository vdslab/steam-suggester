...
const StreamedList = (props: Props) => {
  ...
  const [filteredStreamerList, setFilteredStreamerList] = useState<
    StreamerListType[]
  >([]);
  const [isLoading, setIsLoading] = useState<string[]>([]);
  const [showLimitAlert, setShowLimitAlert] = useState<boolean>(false); // 配信者追加制限アラート
  const [showDataNotFoundAlert, setShowDataNotFoundAlert] =
    useState<boolean>(false); // データ未発見アラート
    ...
    try {
      ...
      const streamedGameList = nodes.reduce((acc: StreamerListType[], node) => {
        ...
        relevantStreamIds.forEach((gameName) => {
          if (gameName === node.title) {
            // gameName と一致
            const existingGame = acc.find(
              (g) =>
                g.name === updatedData.name &&
                g.platform === updatedData.platform
            );
            if (existingGame) {
        ...
        (newGame) =>
          !streamerIds.some(
            (existingGame) =>
              ...
              existingGame.platform === newGame.platform
          )
      );
      ...
      if (newGames.length > 0) {
        ...
        const isAlreadyAdded = streamerIds.some(
          (existingGame) =>
            existingGame.name === updatedData.name &&
            existingGame.platform === updatedData.platform
        );
        if (isAlreadyAdded) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const twitchResponse = await fetch(
        ...
        }/api/details/getTwitchStreamerInf/${encodeURIComponent(
          searchStreamerQuery
        )}`
      );
      ...
      // YouTube API
      const youtubeResponse = null;
      let youtubeData = null;
      //   const youtubeResponse = await fetch(
      //     `${
      //     process.env.NEXT_PUBLIC_CURRENT_URL
      //   }/api/details/getYoutuberInf/${encodeURIComponent(searchStreamerQuery)}`
      // );
      ...
      // TwitchとYouTubeのデータを統合 (youtube削除済み保留)
      // const combinedData = [...twitchData, ...youtubeData];
      const combinedData = [...twitchData];

      // viewer_count順に並び替え（数字のみ比較）
      const sortedData = combinedData.sort((a, b) => {
        const viewerA =
          typeof a.viewer_count === "number"
            ? a.viewer_count
            : parseInt(a.viewer_count) || 0;
        const viewerB =
          typeof b.viewer_count === "number"
            ? b.viewer_count
            : parseInt(b.viewer_count) || 0;
        ...
        {isSearching ? (
          ...
          filteredStreamerList.length > 0 && (
            <div className="bg-gray-700 p-2 rounded-lg mt-0 relative max-h-40 overflow-y-auto">
              <h2 className="text-gray-400 text-xs font-semibold mb-2">
                検索結果
              </h2>
              <div>
                    ...
                    <div className="flex items-center">
                      ...
                      <button
                        ...
                        disabled={
                          ...
                          streamerIds.length >= MAX_STREAMERS
                        }
                        aria-disabled={
                          isLoading.includes(streamer.id) ||
                          streamerIds.length >= MAX_STREAMERS
