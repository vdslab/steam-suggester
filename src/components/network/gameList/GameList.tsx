...
const GameList = (props: Props) => {
  ...
  useEffect(() => {
    ...
    if (searchQuery === "") {
      ...
      const filteredNodes = nodes.filter((game) =>
  ...
  const showNoResultsMessage =
    searchQuery !== "" && filteredNodeList.length === 0;
  const showAddGameMessage =
    showNoResultsMessage && filteredSteamList.length > 0;
            ...
            {nodes.length > 0 ? (
              <div className="space-y-2">
                {nodes.map((node: NodeType) => {
                  ...
                  // const { rankColor } = selectColor(node.index + 1); // ランクカラーの選択を削除
                  ...
                  return (
                    <div
                      ...
                      onClick={() => setSelectedIndex(node.index)} // ノード選択時にインデックスを設定
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center">
                          ...
                          <div className="text-white p-2">{node.title}</div>
                        </div>
                        {isUserAdded && (
                          <DeleteIcon
                            className="cursor-pointer hover:bg-gray-600 rounded"
                        ...
                        <div className="mt-2" ref={selectedDetailRef}>
                          ...
                          <div className="flex items-center">
                            <span className="text-sm mb-1">
                              開発者: {node.developerName}
                            </span>
                          </div>
                          <div className="flex items-center">
                            <span className="text-sm mb-1">
                              発売日: {node.releaseDate}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center">
                            <div>価格: </div>
                            {node.salePrice &&
                            parseInt(node.salePrice, 10) < node.price ? (
                              <>
