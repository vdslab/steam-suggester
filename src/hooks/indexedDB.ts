...
if (typeof window !== "undefined") {
  ...
  request.onupgradeneeded = function (event: IDBVersionChangeEvent) {
    ...
    const networkFilterStore: IDBObjectStore = db.createObjectStore(
      "networkFilter",
      { keyPath: "id" }
    );
    ...
    if (!db.objectStoreNames.contains("userAddedGames")) {
      ...
      userAddedGamesStore.createIndex("steamGameId", "steamGameId", {
        unique: false,
      });
    }
...
export async function getFilterData(): Promise<Filter | null> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const objectStore: IDBObjectStore =
      transaction.objectStore("networkFilter");
...
export async function getGameIdData(): Promise<string[] | null> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const objectStore: IDBObjectStore =
      transaction.objectStore("userAddedGames");
    const request: IDBRequest = objectStore.get("unique_id");
...
export async function getSliderData(): Promise<SliderSettings | null> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const objectStore: IDBObjectStore =
      transaction.objectStore("sliderSettings");
    const request: IDBRequest = objectStore.get("unique_id");
