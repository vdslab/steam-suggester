import {
  ...
  RollupType,
  SteamReviewHistogramApiType,
} from "@/types/api/countSteamReviewsType";
...
export async function GET(req: Request, { params }: Params) {
  ...
  const response = await fetch(
    `https://store.steampowered.com/appreviewhistogram/${gameID}?json=1`
  );
  ...
  const reviewsCount: GetSteamAllReviewsResponse[] = data.results.rollups.map(
    (rollup: RollupType) => ({
